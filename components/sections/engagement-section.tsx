...
export function EngagementSection() {
  ...
  return (
    <section id="engagement-formats" className="relative overflow-hidden py-14 md:py-24">
      ...
      <div className="section-shell relative z-10 space-y-7 md:space-y-9">
            ...
            <Reveal key={format.title} delay={80 + index * 120}>
              <Card className="interactive-card h-full overflow-hidden border-brand-burgundy/15 bg-white/75 hover:border-brand-gold/55 hover:shadow-premium">
                <CardHeader className="space-y-4 border-b border-brand-burgundy/10">
                  <CardTitle className="flex items-center gap-2.5 text-xl tracking-[0.06em] text-brand-burgundy sm:text-2xl">
                    {(() => {
                ...
                <CardContent className="space-y-5">
                  <ul className="grid gap-2.5 text-brand-burgundy sm:grid-cols-2">
                    {format.activities.map((activity) => (
                      <li key={activity} className="interactive-card rounded-lg border border-brand-burgundy/10 bg-brand-ivory/55 p-2.5 sm:p-3">
                        <div className="flex items-center gap-2.5">
                          ...
                          <span className="text-sm leading-5 md:text-base">{activity}</span>
                        </div>
                      </li>
                    ))}
                  </ul>
        ...
        <Reveal delay={220}>
          <Card className="interactive-card border-brand-gold/35 bg-white/75 hover:border-brand-gold/60">
            <CardHeader className="border-b border-brand-burgundy/10">
              <CardTitle className="text-xl tracking-[0.06em] text-brand-burgundy sm:text-2xl">{siteContent.engagement.handleTitle}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-7">
              <div className="relative rounded-2xl border border-brand-gold/35 bg-white/65 p-4 md:p-6 xl:p-7">
                <div className="grid grid-cols-2 gap-2.5 md:hidden">
                  {flowSteps
                    ...
                    .map((step, index) => {
                      ...
                      return (
                        <Reveal key={step.label} delay={120 + index * 70}>
                          <div className="snake-node min-h-[7rem] p-3">
                            <div className="mb-2 flex items-center justify-between">
                              <p className="text-[11px] font-bold tracking-[0.08em] text-brand-gold">0{step.number}</p>
                              <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-brand-gold/55 bg-white">
                                <Icon className="h-3.5 w-3.5 text-brand-burgundy" />
                              </div>
                            </div>
                            <p className="text-[12px] font-medium leading-4 text-brand-burgundy">{step.label}</p>
                          </div>
                        </Reveal>
                      );
                    })}
                ...
                <div className="hidden space-y-5 md:block xl:hidden">
                  {flowSteps
                    .slice()
                    .sort((a, b) => a.number - b.number)
                    .map((step, index) => {
